...
public sealed class DreamTeamWebApplicationFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    private const string MinioAccessKey = "minioadmin";
    private const string MinioSecretKey = "minioadmin";
    private const string MinioBucket = "dreamteam-integration-test-uploads";
    ...
    private void RewireStorageForS3(IServiceCollection services)
    {
        var toRemove = services
            .Where(d => d.ServiceType == typeof(DreamTeam.Framework.Storage.Services.IStorageService)
                     || d.ServiceType == typeof(DreamTeam.Framework.Storage.Local.LocalStorageService)
                     || d.ServiceType == typeof(DreamTeam.Framework.Storage.S3.S3StorageService)
                     || d.ServiceType == typeof(IAmazonS3))
            .ToList();
        foreach (var d in toRemove) services.Remove(d);

        services.Configure<DreamTeam.Framework.Storage.S3.S3StorageOptions>(opts =>
        {
        ...
        services.AddTransient<DreamTeam.Framework.Storage.S3.S3StorageService>();
        services.AddTransient<DreamTeam.Framework.Storage.Services.IStorageService>(sp =>
            sp.GetRequiredService<DreamTeam.Framework.Storage.S3.S3StorageService>());
    }
