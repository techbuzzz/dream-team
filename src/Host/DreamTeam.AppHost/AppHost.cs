...
// Per-app prefix from the AppHost assembly name (DreamTeam.AppHost -> dreamteam); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var postgres = postgresServer.AddDatabase("dreamteam-db");
...
builder.AddContainer("redis-insight", "redis/redisinsight", "latest")
    ...
    .WithEnvironment("RI_REDIS_ALIAS0", "dreamteam-cache")
...
const string MinioBucket = "dreamteam-uploads";
...
var migrator = builder.AddProject<Projects.DreamTeam_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "DreamTeam.Migrations.PostgreSQL")
...
var demoSeeder = builder.AddProject<Projects.DreamTeam_DbMigrator>($"{appPrefix}-demo-seeder")
    .WithReference(postgres)
    .WaitFor(postgres)
...
// API Service
var api = builder.AddProject<Projects.DreamTeam_Api>($"{appPrefix}-api")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WaitFor(redis)
