...
if (string.IsNullOrWhiteSpace(builder.Configuration["JwtOptions:SigningKey"]))
{
    ...
    {
        ["JwtOptions:SigningKey"] = "dreamteam-dbmigrator-placeholder-never-mints-tokens-32+",
        ["JwtOptions:Issuer"] = builder.Configuration["JwtOptions:Issuer"] ?? "dreamteam.local",
...
builder.Services.AddSingleton<DreamTeam.Framework.Jobs.Services.IJobService, NoOpJobService>();
...
// DemoSeeder is removed (FSH-only demo data for acme/globex tenants). MVP-1
// has no demo seed; the seed-demo verb is a no-op until DreamTeam seeds land.
...
try
{
    ...
    // ── Step 3 — demo seed (verb: `seed-demo`) — DISABLED for MVP-1 ──────
    // The FSH demo seeder (acme/globex tenants, demo users, demo content) is
    // removed with the FSH modules. The verb is still recognised but is a no-op
    // until DreamTeam seeds land as a separate workstream.
    if (cli.Command == "seed-demo")
    {
        await Console.Out.WriteLineAsync(
            "[demo-seed] DreamTeam MVP-1 has no demo seed. seed-demo is a no-op until the seed workstream lands.")
            .ConfigureAwait(false);
    }
