...
dotnet run --project src/Host/DreamTeam.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/DreamTeam.DbMigrator -- apply --catalog-only

# Preview what would run without touching the database.
dotnet run --project src/Host/DreamTeam.DbMigrator -- list-pending

# Apply migrations AND run idempotent seed data.
dotnet run --project src/Host/DreamTeam.DbMigrator -- apply --seed

# Just the seed step (assumes schema is already current).
dotnet run --project src/Host/DreamTeam.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/DreamTeam.DbMigrator -- seed-demo
```
...
as `DreamTeam.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `DreamTeam.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/DreamTeam.DbMigrator -c Release \
  /t:PublishContainer /p:ContainerRepository=dreamteam-db-migrator
```
...
spec:
  ...
  template:
    spec:
      ...
      containers:
        - name: migrator
          image: ghcr.io/your-org/dreamteam-db-migrator:{{ .Chart.AppVersion }}
          args: ["apply"]
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: DreamTeam.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/DreamTeam.AppHost` —
